from __future__ import annotations

import pytest

import loza


def test_duplicate_emit_is_idempotent_and_finish_after_emit_is_typed() -> None:
    logger = loza.New(loza.Test("checkout").with_sink(loza.NoopSink()))
    ctx = logger.start_event(loza.Params(event="state.machine"))
    logger.finish(ctx, "success")
    first = logger.emit(ctx)
    second = logger.emit(ctx)

    assert first == second
    with pytest.raises(loza.EventClosedError):
        logger.finish(ctx, "success")


def test_validation_failure_does_not_mark_emitted() -> None:
    logger = loza.New(loza.Test("checkout"))
    ctx = logger.start_event(loza.Params(event="bad", kind="bad-kind"))

    with pytest.raises(loza.EventValidationError):
        logger.emit(ctx)

    assert not ctx.is_emitted()
    assert ctx.event_state == "failed_validation"


def test_created_transitions_to_active_on_enrich() -> None:
    logger = loza.New(loza.Test("checkout").with_sink(loza.NoopSink()))
    ctx = logger.start_event(loza.Params(event="state.transition"))

    assert ctx.event_state == "created"
    logger.enrich(ctx, loza.String("user.id", "u1"))
    assert ctx.event_state == "active"
