...
opaque access ID (`lz_pub_...` with no password). A DSN without userinfo is
...
LOZA_API_KEY=lz_sec_dev_k_xxx

# Production
LOZA_DSN=loza://collector.example.com/payments?env=prod
LOZA_API_KEY=lz_sec_prod_k_xxx
...
- A public DSN sends `Authorization: Basic <base64(lz_pub_<random>:)>`. Its opaque public access ID is a bearer capability and MUST be redacted and protected like a secret.
...
export LOZA_API_KEY=lz_sec_prod_k_xxx
...
export LOZA_DSN='loza://lz_pub_<random>@collector.example.com/my-app?env=prod'
...
| `loza://lz_pub_<random>@host/collector` | Valid public opaque access ID DSN      |
| `loza://user@host/collector`    | Public access ID must begin `lz_pub_`          |
