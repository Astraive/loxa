// End-to-end test: loza-rs SDK -> HTTPBatchSink -> loza-collector.
//
// Requires a live collector at LOZA_TEST_COLLECTOR_URL (default
// http://127.0.0.1:9308) and its ingest token in LOZA_API_KEY.
...
fn collector_url() -> String {
    env::var("LOZA_TEST_COLLECTOR_URL").unwrap_or_else(|_| COLLECTOR_URL.to_string())
}
...
fn config_option_with_collector_endpoint_auto_wires_httpbatch() {
    let cfg = loza::NewWith(vec![WithCollectorEndpoint(COLLECTOR_URL)]);
...
fn explicit_file_sink_preserved_alongside_collector_endpoint() {
    let cfg = Config::dev("explicit")
        .with_sink(loza::FileSink("/tmp/loza-e2e.log"))
...
#[test]
#[ignore = "requires LOZA_TEST_COLLECTOR_URL and LOZA_API_KEY"]
...
fn raw_envelope_accepted_by_collector() {
    ...
    let envelope = serde_json::json!({
        "api_version": "v1",
        "source": {"sdk": "loza-rs", "version": "0.2.0", "service": "e2e_raw"},
        "events": [{
...
fn batch_envelope_accepted_by_collector() {
    ...
    let envelope = serde_json::json!({
        "api_version": "v1",
        "source": {"sdk": "loza-rs", "version": "0.2.0", "service": "e2e_batch"},
        "events": events
    });
...
fn sdk_emit_produces_valid_envelope() {
    ...
    let client = loza::core::client::CollectorHttpClient::new(COLLECTOR_URL);
    ...
    assert_eq!(envelope["source"]["sdk"], "loza-rs");
    client
...
fn http_event_with_trace_context_accepted() {
    ...
    let envelope = serde_json::json!({
        "api_version": "v1",
        "source": {"sdk": "loza-rs", "version": "0.2.0", "service": "e2e_http"},
        "events": [{
...
fn error_event_with_error_details_accepted() {
    ...
    let envelope = serde_json::json!({
        "api_version": "v1",
        "source": {"sdk": "loza-rs", "version": "0.2.0", "service": "e2e_error"},
        "events": [{
...
fn enriched_event_with_attrs_accepted() {
    ...
    let envelope = serde_json::json!({
        "api_version": "v1",
        "source": {"sdk": "loza-rs", "version": "0.2.0", "service": "e2e_attrs"},
        "events": [{
...
fn collector_rejects_invalid_envelope() {
    ...
    let bad_envelope = serde_json::json!({
        "source": {"sdk": "loza-rs", "version": "0.2.0", "service": "test"},
