...
type fakeCortexBridgeServer struct {
	...
	mu      sync.Mutex
	batches []*loxav1.IngestBatchRequest
}

func (f *fakeCortexBridgeServer) IngestBatch(_ context.Context, req *loxav1.IngestBatchRequest) (*loxav1.IngestBatchResponse, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.batches = append(f.batches, req)
	return &loxav1.IngestBatchResponse{Status: "accepted", Count: int32(len(req.Events))}, nil
}
...
func (f *fakeCortexBridgeServer) lastBatch() *loxav1.IngestBatchRequest {
	f.mu.Lock()
	defer f.mu.Unlock()
	if len(f.batches) == 0 {
		return nil
...
func startFakeCortexBridge(t *testing.T) (string, *fakeCortexBridgeServer, func()) {
	...
	loxav1.RegisterCortexServiceServer(srv, fake)
	go func() {
		_ = srv.Serve(lis)
	}()
