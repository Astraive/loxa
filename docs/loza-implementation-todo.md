# Loza Implementation Todo

> **Rule**: Only export `loza`. No bare `logger` export.  
> `loza.<method>()` — default global client  
> `createLoza(config)` — custom client factory  
> `loza.alias(name)` — same-config child alias  
...
The CLI (`loza keys create/revoke/rotate`) calls client functions, but the collector server has no runtime key management HTTP endpoints. Keys are configuration-only.
...
| 88 | Same `createLoza` naming | ✅ All 4 SDKs |
...
| # | File | Issue |
|---|---|---|
| 91 | `sdks/py/src/loza/core/client.py` | Duplicate Logger class (477 lines), unused |
| 92 | `sdks/py/src/loza/core/options.py` | Duplicate With* functions, unused |
| 93 | `sdks/py/src/loza/config/config.py` | Duplicate of `core/config.py`, less complete |
...
# Factory
createLoza(config) / create_loza(config) / CreateLoza(config)
...
Current state:
- ✅ JS: Only `loza`, `createLoza`, `Loza` class
- ✅ Python: Only module-level `loza.*` functions, `create_loza`, `Logger` class
- ✅ Go: Only package-level `loza.*` functions, `CreateLoza`/`New`
- ✅ Rust: Only crate-level `loza::*` functions, `create_loza`, `Loza::new`
