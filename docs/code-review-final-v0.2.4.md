...
Files:
- `E:/astraive/loza/loza/sdks/py/src/loza/core/http_client.py:127` -- `sdk_version: str = "0.2.0"`
- `E:/astraive/loza/loza/sdks/py/src/loza/core/pipeline.py:317` -- `"version": "0.2.0"`
- `E:/astraive/loza/loza/sdks/py/src/loza/sinks/httpbatch/httpbatch.py:38` -- `sdk_version: str = "0.2.0"`
- `E:/astraive/loza/loza/sdks/js/src/sinks/standard-sinks.ts:197` -- `this.sdkVersion = opts.sdkVersion || '0.2.0'`
- `E:/astraive/loza/loza/sdks/js/src/collector/client.ts:69` -- `buildIngestEnvelope('loza-js', '0.2.0', ...)`
- `E:/astraive/loza/loza/sdks/rs/src/core/client.rs:175` -- `sdk_version: "0.2.0".to_string()`
...
Files:
- `E:/astraive/loza/loza/collector/Dockerfile:41` -- `EXPOSE 9090 9092`
- `E:/astraive/loza/loza/collector/deploy/docker/Dockerfile.collector:40` -- `EXPOSE 9090 9092`
- `E:/astraive/loza/loza/collector/deploy/Dockerfile.collector:40` -- `EXPOSE 9090 9092`

Issue:
The canonical port map (`docs/ports.md`) defines collector HTTP on port 9308 and collector gRPC on 9309. The defaults YAML (`collector/loza-collector.defaults.yaml`) confirms `addr: ":9308"`. But all three collector Dockerfiles `EXPOSE 9090 9092`, which are the old pre-remap ports. While EXPOSE is informational (the app binds to 9308 regardless), this misleads container orchestrators, Docker Compose examples, and developers reading the Dockerfile.
...
File: `E:/astraive/loza/loza/cortex/configs/Dockerfile:47-50`

Issue:
The Cortex Dockerfile exposes `8080 9090` and sets `CORTEX_SERVER_PORT=8080`. The canonical port map defines Cortex HTTP on 9312 and gRPC on 9313. The `cortex/configs/loza-cortex.defaults.yaml` confirms `port: 9312`. This means container deployments using this Dockerfile will bind cortex to port 8080 instead of 9312, breaking the entire port allocation scheme.
...
#### [HIGH] Lozana package.json version stuck at 0.2.3

File: `E:/astraive/loza/lozana/package.json:3` -- `"version": "0.2.3"`

Issue:
The `lozana/lozana.yaml` metadata file correctly shows `version: 0.2.4`, but `package.json` (which is the source of truth for npm and build tooling) still says `0.2.3`. The bump script does not include `lozana/package.json` because Lozana is a separate repo (`lozana/` is not at the monorepo root).

Why existing guards do not catch it:
The bump script's `FILES` array does not include `lozana/package.json`. Lozana's `lozana.yaml` was manually bumped but `package.json` was missed.

Fix:
Either add `lozana/package.json` to the bump script or manually bump it to `0.2.4`.
...
Files:
- `E:/astraive/loza/loza/collector/Dockerfile` -- no HEALTHCHECK
- `E:/astraive/loza/loza/collector/deploy/docker/Dockerfile.collector` -- no HEALTHCHECK
- `E:/astraive/loza/loza/collector/deploy/docker/Dockerfile.worker` -- no HEALTHCHECK
- `E:/astraive/loza/loza/cortex/configs/Dockerfile` -- no HEALTHCHECK

Issue:
The Lozana and spec service Dockerfiles have proper HEALTHCHECK directives, but all collector and cortex Dockerfiles lack them. Docker/containerd health checks are critical for orchestrators to detect unresponsive containers and trigger restarts. Without them, a hung collector or cortex process will receive traffic indefinitely.
...
File: `E:/astraive/loza/loza/CHANGELOG.md`
...
File: `E:/astraive/loza/loza/cli/CHANGELOG.md`
...
File: `E:/astraive/loza/loza/cortex/SECURITY.md:45`
...
File: `E:/astraive/loza/loza/cortex/README.md:3`
...
Files:
- `E:/astraive/loza/loza/collector/deploy/helm/loza/values.yaml:85` -- `ingest: /ingest`
- `E:/astraive/loza/loza/collector/loza-collector.defaults.yaml:46` -- `ingest: /events`
- `E:/astraive/loza/loza/collector/deploy/helm/loza/values.yaml:86` -- `health: /healthz`
- `E:/astraive/loza/loza/collector/loza-collector.defaults.yaml:47` -- `health: /health`
...
File: `E:/astraive/loza/loza/RELEASE.md`
...
File: `E:/astraive/loza/loza/docs/release-notes.md:151-165`

Issue:
Install instructions reference `docker pull astraive/loza:0.2.0`, `helm install loza loza/loza --version 0.2.0`, `loza==0.2.0` (PyPI), `loza = "0.2.0"` (Crates.io). All should reference 0.2.4.
...
File: `E:/astraive/loza/loza/docs/getting-started.md:3`
...
- `E:/astraive/loza/loza/sdks/go/src/core/ingest_envelope_fixture_test.go:52,91`
- `E:/astraive/loza/loza/sdks/go/src/core/config_file_test.go:189,210,211`
- `E:/astraive/loza/loza/sdks/py/tests/test_ingest_envelope_fixture.py:37`
- `E:/astraive/loza/loza/sdks/py/tests/test_non_mutating_normalization.py:37`
- `E:/astraive/loza/loza/sdks/js/tests/conformance.test.ts:41,46`
- `E:/astraive/loza/loza/sdks/rs/tests/ingest_envelope_fixture.rs:86`
- `E:/astraive/loza/loza/sdks/rs/tests/domain_helpers_test.rs:346`
...
File: `E:/astraive/loza/loza/docs/configuration.md:275`
...
File: `E:/astraive/loza/loza/docs/specification-and-release-index.md:5`
...
#### [LOW] Collector deploy/loza-collector.yaml route inconsistency

File: `E:/astraive/loza/loza/collector/deploy/loza-collector.yaml:19-22`
...
File: `E:/astraive/loza/loza/sdks/go/tests/conformance/api_surface_test.go:98`

Issue:
Conformance test uses `loza.AppVersion("0.2.0")` which sets the service version (not SDK version). This is test data, not a version tracking issue.
...
File: `E:/astraive/loza/loza/docs/release-notes.md:145-146`
...
1. **Consistent version in loza*.yaml metadata files**: All 8 component metadata files (`collector/loza.metadata.yaml`, `cortex/loza-cortex.metadata.yaml`, `cli/loza-cli.yaml`, `sdks/go/loza-go.yaml`, `sdks/py/loza-py.yaml`, `sdks/rs/loza-rs.yaml`, `sdks/js/loza-js.yaml`, `lozana/lozana.yaml`) correctly show `version: 0.2.4`.

2. **Consistent version in Go binaries**: All Go binaries (`collector/cmd/loza-collector/main.go`, `collector/cmd/loza-worker/main.go`, `collector/cmd/loza-loadgen/main.go`, `cortex/cmd/server/main.go`, `cli/cmd/loza/main.go`) correctly show `var version = "0.2.4"`.
...
6. **DSN parsing implemented in all 4 SDKs**: Go, Python, JavaScript, and Rust all have `loza://` DSN parsing with tests. The Go SDK has comprehensive DSN tests covering edge cases.
...
9. **Comprehensive CI coverage**: 18 CI workflows covering collector, cortex, LQL, all SDKs, Lozana, spec validation, conformance, and benchmarks.
...
11. **Port map documentation is excellent**: `docs/ports.md` clearly defines all ports, explains the 4-layer config system, and documents the `loza://` DSN format.

12. **Consistent non-root containers**: All Dockerfiles create a `loza` (or `lozana`) user and run as non-root.

13. **Collector Dockerfiles correctly handle config via entrypoint**: `docker-entrypoint.sh` supports `LOZA_CONFIG` (inline YAML) and `LOZA_CONFIG_FILE` (path) env vars.
...
3. **P1 -- Fix Lozana package.json version**: Bump to 0.2.4.
...
8. **P2 -- Expand bump script coverage**: Add SDK source code defaults, Dockerfile EXPOSE, Lozana package.json, and documentation files to the bump script.
...
| Lozana | 0.2.3/0.2.4 | PASS | PASS | PASS | PASS | 75/100 |
