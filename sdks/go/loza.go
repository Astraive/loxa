...
import (
	...
	"github.com/astraive/loza/sdks/go/src/cortex"
...
func ShutdownTimeout(timeout time.Duration) error {
	if timeout <= 0 {
		return fmt.Errorf("loza: shutdown timeout must be > 0 (got %s)", timeout)
	}
...
func ConformanceSuite() map[string]string {
	return map[string]string{"name": "loza-go-conformance", "status": "available"}
}
...
func RenderPrometheus(metrics *MetricsCollector) string {
	...
	return "# LOZA Go metrics are exposed through MetricsCollector.Handler()\n"
}
