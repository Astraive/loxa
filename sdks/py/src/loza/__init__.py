...
from .core import (  # noqa: F403
    ...
    LOZA_SPEC_VERSION,
    load_layered_config,
    new_client as _new_client_core,
    WithService,
    WithVersion,
    WithEnvironment,
    WithSink,
...
# Re-export AttrAny as loza.Any (hides typing.Any at top level)
from .core import AttrAny as _loza_Any, MetricsCollector, MetricsSnapshot
...
def create_loza(service: str = "", **kwargs: Any) -> Logger:
...
def from_request(req: Any, logger: Optional[Logger] = None) -> EventContext:
    ...
    route = getattr(req, "route", None) or path or (req.get("route", path) if isinstance(req, dict) else path)
    request_id = ""
    trace_id = ""
    user_agent = ""
    referer = ""
    ...
    log.set(ctx, "http.method", str(method).upper())
    log.set(ctx, "http.path", str(path))
    log.set(ctx, "http.route", str(route))
    if request_id:
        log.set(ctx, "request.id", str(request_id))
    if trace_id:
        log.set(ctx, "trace.id", str(trace_id))
    if user_agent:
        log.set(ctx, "http.user_agent", str(user_agent)[:512])
    if referer:
        log.set(ctx, "http.referer", str(referer).split("?")[0])
    return ctx


# ---------------------------------------------------------------------------
# Go-style uppercase aliases — import loza as logger; logger.Enrich(...)
# ---------------------------------------------------------------------------
Configure = configure
Default = default
New = new
...
# ---------------------------------------------------------------------------
# Attribute helpers — canonical LOZA keys
# ---------------------------------------------------------------------------
...
def NewMetricsCollector(namespace: str = "loza_sdk", *, buffer_capacity: int = 0):
