...
def looks_like_loxa_event(payload: dict[str, Any]) -> bool:
...
def validate_event_payload_detailed(payload: dict[str, Any], strict: bool = False) -> list[ValidationError]:
    ...
    if normalized.get('schema_version') not in {LOXA_SPEC_VERSION}:
    ...
    if normalized.get('event_version') not in {LOXA_EVENT_VERSION}:
