...
CONTRACT = {
    ...
    "required_fields": ["schema_version", "event_version", "event_id", "timestamp", "service", "event", "kind"],
    ...
    "enums": {
        "schema_versions": ["v1"],
        "event_versions": ["v1"],
        "kinds": ["event", "http", "job", "queue", "cli", "cron", "log", "checkpoint", "agent", "ai"],
        ...
        "partial_reasons": ["not_finished", "process_exit", "timeout", "panic", "collector_unavailable"],
        "event_states": [
            "created",
            "active",
            "finished",
        ...
        "source_sdks": ["loza-cli", "loza-go", "loza-py", "loza-rs"],
    },
    "collector_statuses": ["accepted", "partial", "rejected", "invalid", "quarantined"],
    "collector_ack_statuses": ["accepted", "rejected", "invalid", "quarantined"],
    "aliases": {"event_type": "event"},
    "alias_policy": {"strict_mode": "reject_before_normalization", "loose_mode": "normalize_then_use"},
    "strict_mode": {
    ...
    "fixtures": {
        ...
        "loose_only_valid": ["valid/loose_event_type_alias.json"],
        "invalid": [
        ...
        "strict_only_invalid": ["invalid/strict_event_type_alias.json"],
        "invalid_ingest": ["invalid/bad_ingest_events_array.json"],
        "invalid_collector_response": ["invalid/bad_collector_status.json"],
        "invalid_limits": ["invalid/oversized.json"],
        "emitted_shape": ["emitted-shape/structured_http_success.json"],
...
class CollectorResponse:
    ...
    def permanent_failure(self) -> tuple[bool, str]:
        ...
        return (
            True,
            self.error or self.reason or f"accepted={self.accepted} rejected={self.rejected} invalid={self.invalid}",
        )
...
def build_ingest_envelope(
    events: Iterable[dict[str, Any]], sdk_name: str, sdk_version: str, service: str
) -> dict[str, Any]:
    events_list = [normalize_event_aliases(e)[0] for e in events]
    return {
        "api_version": LOZA_INGEST_API_VERSION,
        "source": {"sdk": sdk_name, "version": sdk_version, "service": service.strip() or _infer_service(events_list)},
        "events": events_list,
    }
    ...
    if strict:
        for key in normalized:
            if key not in ALLOWED_TOP_LEVEL_FIELDS:
                errors.append(
                    ValidationError(
                        field=key, code="unknown_strict_field", message=f'field "{key}" is not allowed by strict schema'
                    )
                )
    ...
    if normalized.get("schema_version") not in {LOZA_SPEC_VERSION}:
        errors.append(
            ValidationError(
                ...
                message=f'field "schema_version" has unsupported value "{normalized.get("schema_version")}"',
            )
        )
    if normalized.get("event_version") not in {LOZA_EVENT_VERSION}:
        errors.append(
            ValidationError(
                ...
                message=f'field "event_version" has unsupported value "{normalized.get("event_version")}"',
            )
        )
        ...
        raise ValidationErrors(
            [
                ValidationError(
                    field="payload",
                    code="too_large",
                    message=f"payload exceeds max_event_size_bytes ({len(trimmed)} > {MAX_EVENT_BYTES})",
                )
            ]
        )
    try:
    ...
    raise ValidationErrors(
        [
            ValidationError(
                ...
                message="payload must be a JSON object, array, wrapper, or NDJSON",
            )
        ]
    )
...
def _req_enum(payload: dict[str, Any], key: str, allowed: set[str], errors: list[ValidationError]) -> None:
    ...
    if isinstance(value, str) and value.strip() and value.strip() not in allowed:
        errors.append(
            ValidationError(field=key, code="invalid_enum", message=f'field "{key}" has unsupported value "{value}"')
        )
...
def _opt_nonneg_int(payload: dict[str, Any], key: str, errors: list[ValidationError]) -> None:
    if key in payload and (not isinstance(payload[key], int) or payload[key] < 0):
        errors.append(
            ValidationError(field=key, code="invalid_integer", message=f'field "{key}" must be a non-negative integer')
        )
...
def _opt_status_code(payload: dict[str, Any], key: str, errors: list[ValidationError]) -> None:
    if key in payload and (not isinstance(payload[key], int) or payload[key] < 100 or payload[key] > 599):
        errors.append(
            ValidationError(field=key, code="invalid_status_code", message=f'field "{key}" must be between 100 and 599')
        )
...
def _req_service(payload: dict[str, Any], errors: list[ValidationError]) -> None:
    ...
    errors.append(
        ValidationError(
            ...
            message='field "service" must be a non-empty string or object with name',
        )
    )
