...
func TestParityManifestMatchesStableLifecycleDoc(t *testing.T) {
	...
	if manifest.Version != "0.0.2" {
		t.Fatalf("expected product parity manifest version 0.0.2, got %q", manifest.Version)
	}
	if manifest.Stability != "product-parity-v0.0.2" {
		t.Fatalf("expected stability product-parity-v0.0.2, got %q", manifest.Stability)
	}
