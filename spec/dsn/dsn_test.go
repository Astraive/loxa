...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		// ── Valid cases ───────────────────────────────────────────────────────
		{
			...
			input:     "loxa://localhost:8080/demo?tls=false",
			valid:     true,
			host:      "localhost",
			port:      8080,
			project:   "demo",
			...
			batchURL:  "http://localhost:8080/events/batch",
			otlpURL:   "http://localhost:8080/otlp/logs",
			tailWSURL: "ws://localhost:8080/tail",
		},
		{
			name:      "localhost default port 8080",
		...
		{
			name:    "127.0.0.1 defaults to tls=false",
		...
		{
			name:    "::1 defaults to tls=false (brackets required)",
			input:   "loxa://[::1]/demo",
			valid:   true,
			host:    "::1",
			port:    80,
