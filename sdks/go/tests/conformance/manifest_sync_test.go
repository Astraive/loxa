...
func TestParityManifestMatchesStableLifecycleDoc(t *testing.T) {
	...
	if manifest.Version != "0.3.1" {
		t.Fatalf("expected product parity manifest version 0.3.1, got %q", manifest.Version)
	}
	if manifest.Stability != "product-parity-v0.3.1" {
		t.Fatalf("expected stability product-parity-v0.3.1, got %q", manifest.Stability)
	}
